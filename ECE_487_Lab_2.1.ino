...
void setup() {
  ...
  // Print zero to Serial Monitor
  printCount(0);
}

void loop() {
  int count = 0;                                // Initialize count to zero
  int switchState = digitalRead(ToggleSwitch);  // current switch reading
  int lastSwitchState = (!switchState);         // set previous reading
  bool blinkMode = switchState;                 // If switch is ON, LEDs 
                                                // should begin in blinkMode
  // Time Variables
  ...
  while(1) {
    ...
    int reading = digitalRead(ToggleSwitch);
    ...
    // Check if reading has been there longer than debounce delay
    if ((millis() - lastDebounceTime) > debounceDelay) {
      ...
      if (reading != switchState) {
        ...
        // Increment count for num of inputs
        count++;

        // Print count to 7-Seg display and Serial Monitor
        printCount(count%16);       // count%16 for rollover
        sevenSegDisplay(count%16);  
        
        // Blink LEDs if switch is on
        if (switchState == HIGH) { 
          ledsOnMessage(); 
          blinkMode = HIGH;       // Update blinkMode
          lastSwitchState = LOW;  // Update switch state
        } 
        
        // Turn off LEDs if switch is off
        else {
          ledsOff();
          blinkMode = LOW;        // Update blinkMode
          lastSwitchState = HIGH; // Update switch state
        }
      }
    }

    // Check blinkMode and blink LEDs if ON
    if(blinkMode) { blinkLeds(); }  
  }
}
...
void initialPrompt() {
  Serial.println("\nPlease enter move Toggle Switch to 'on' "
    "position to begin blinking LEDs. ");
  Serial.println("To turn off LED's, move "
    "the Toggle Switch to 'off' position.");
  Serial.println("\nThe 7-Segment Display and Serial Monitor "
    "will keep track of the ");
  Serial.println("total number of user inputs "
    "that have occurred. (Toggle switch inputs ONLY.)");
}

/********************************************************
 * Function Name:     ledsOnMessage
 * Input Parameters:  void
 * Return Value:      void
 * Purpose:           Sends message to Serial Monitor
 *                    updating user that LEDs have been 
 *                    turned on
 *******************************************************/
void ledsOnMessage() {
...
/********************************************************
 * Function Name:     ledsOff
 * Input Parameters:  void
 * Return Value:      void
 * Purpose:           Turns off both onboard and offboard
 *                    LEDs. Also sends message to Serial
 *                    Monitor letting user know LEDs 
...
/********************************************************
 * Function Name:     blinkLeds
 * Input Parameters:  void
 * Return Value:      void
 * Purpose:           Blinks onboard and offboard LEDs
 ...
 * Return Value:      void
 * Purpose:           Prints the input count to the Serial 
 *                    Monitor in both decimal and 
 *                    hexadecimal values
 *******************************************************/
